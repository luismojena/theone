...
import { WatchlistEntry } from '../src/core/domain.js';

class MockTargetPlatform extends IAnimePlatform {
  get platformName() { return 'mock_target'; }
  async authenticate() {}
  async fetchWatchlist(): Promise<WatchlistEntry[]> { return []; }
...
test('PlatformImporterService maps missing entries', async () => {
  ...
  repo.setMapping('jkanime', 'naruto-slug', 10, 'Naruto', { mal_status: 'Watching' });
  repo.setMapping('jkanime', 'bleach-slug', 20, 'Bleach', { mal_status: 'Completed' });
  
  // Also create one mapping that is ALREADY on mock_target (so it shouldn't be mapped again)
  repo.setMapping('mock_target', 'bleach-mock', 20, 'Bleach', { mal_status: 'Completed' });
