...
export class JKAnimePlatform extends IAnimePlatform {
    ...
    async fetchWatchlist() {
        ...
        return uniqueItems.map(item => new WatchlistEntry(this.platformName, item.slug, item.title, this._mapStatus(item.mal_status), item.episodesWatched || 0));
    }
    ...
    async updateEntryStatus(entry) {
        ...
        if (!animeId)
            throw new Error('Could not find data-anime attribute');
        let tagId = 2; // Completado
        if (entry.status === WatchStatus.WATCHING)
            tagId = 1;
        else if (entry.status === WatchStatus.ON_HOLD)
            tagId = 5;
        else if (entry.status === WatchStatus.DROPPED)
            tagId = 6;
        else if (entry.status === WatchStatus.PLAN_TO_WATCH)
            tagId = 4;
        const payload = new URLSearchParams({
