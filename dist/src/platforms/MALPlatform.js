...
export class MALPlatform extends IAnimePlatform {
    ...
    async fetchWatchlist() {
        if (!this.username)
            throw new Error("Username required for MALPlatform");
        const entries = [];
        let offset = 0;
        let hasMore = true;
        const statusNumMap = {
        ...
        while (hasMore) {
            ...
            for (const item of items) {
                if (item.anime_id) {
                    entries.push(new WatchlistEntry(this.platformName, item.anime_id.toString(), item.anime_title, statusNumMap[item.status] || WatchStatus.WATCHING, item.num_watched_episodes || 0, Number(item.anime_id)));
                }
            }
            if (items.length < 300) {
                hasMore = false;
