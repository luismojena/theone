...
- **`src/core/`**: Define la columna vertebral estructural de la aplicación. Incluye `domain.ts` (Entidades/Interfaces como `WatchlistEntry` y `MappingEntry`), `interfaces.ts` (Interfaces de plataformas), `HttpClient.ts` (Abstracción de peticiones HTTP), y `typeGuards.ts` (Validadores de JSON/Errores en tiempo de ejecución tipo Zod).
