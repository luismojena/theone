...
- **Strategy Pattern**: All platforms implement the `IAnimePlatform` abstract class, making adding new platforms trivial without modifying core sync logic.
...
- **`src/core/`**: Defines the structural backbone of the app. Includes `domain.ts` (Entities/Interfaces like `WatchlistEntry` and `MappingEntry`), `IAnimePlatform.ts` (Abstract Platform definition), and `typeGuards.ts` (Zod-like JSON/Error runtime validators).
