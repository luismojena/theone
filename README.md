...
The tool uses `migrate.js` as the central entry-point command router.
...
```bash
node migrate.js sync-jkanime [--force] [--autoskip]
```
* **Zero-Prompt Bulk Mode**: Run `node migrate.js sync-jkanime --force --autoskip` to automatically sync all shows hands-free using your `.env` credentials.
...
├── migrate.js                # Main unified CLI router
